...
struct List *build_tree(const char starting_directory[], size_t max_depth) {
   ...
   struct stat st;
   if (lstat(starting_directory, &st) == -1){
        printf("lstat error\n");
        return NULL;
   }   
   ...
   if (S_ISREG(st.st_mode)) {
        ...
        if(d) {
            ...
            printf("File type or something is not supported\n");
            free(node->name);
            free(node);
            return NULL;
         }
