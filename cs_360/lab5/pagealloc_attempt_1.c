...
static char *bookkeep = NULL; 	  // pointer to the bookkeeping region of the memory pool
...
// two bits are allocated per page, 0 = take, 1 = last block we have to keep track of.

// use unsigned char (or uint8_t) for byte-wise ops
static inline bool test_bit(const unsigned char *bitset, size_t index) {
    size_t byte   = index / PAGES_PER_BYTE;
    size_t shift  = (index % PAGES_PER_BYTE) * BITS_PER_PAGE;
    unsigned char v = bitset[byte];
    return ((v >> shift) & 0x1u) != 0;   // adapt if you use 2-bit states
}

static inline void set_bit(unsigned char *bitset, size_t index) {
    size_t byte  = index / PAGES_PER_BYTE;
    size_t shift = (index % PAGES_PER_PAGE) * BITS_PER_PAGE;
    bitset[byte] |= (unsigned char)(0x1u << shift);
}

static inline void clear_bit(unsigned char *bitset, size_t index) {
    size_t byte  = index / PAGES_PER_BYTE;
    size_t shift = (index % PAGES_PER_PAGE) * BITS_PER_PAGE;
    bitset[byte] &= (unsigned char)~(0x1u << shift);
}
...
bool page_init(size_t pages) {
	...
	bookkeep = (char*)addr; // set the BK pointer to the beginning of the mmap memory region.
	...
	for (size_t i = 0; i < BK_PAGES * PAGE_SIZE; i++) {
		bookkeep[i] = 0;
	}
...
void *page_alloc(size_t pages) {
	...
	size_t free_pages = 0; // set a counter for the amount of free pages that we find
	size_t start_page = 0; // this is the starting index of our bookkeeping region
	...
	for (size_t i = 0; i < num_page; i++) {
		...
		if (found) {
			for(size_t j = 0; j < pages; j++) {
				set_bit(bookkeep, i + j); // marks the page as taken
			}
...
void page_free(void *addr) {
	...
	for(size_t i = page_index; i < num_page; i++) {
		clear_bit(bookkeep, i);
		if (test_bit(bookkeep, i + 1)) {
